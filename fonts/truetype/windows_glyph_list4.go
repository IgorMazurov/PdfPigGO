...
// NumberOfMacGlyphs is the number of standard Mac glyph names.
const NumberOfMacGlyphs = 258

// MacGlyphNames are the 258 standard Mac glyph names used in 'post' format 1 and 2.
...
// MacGlyphNamesIndices maps each standard Mac glyph name to its index.
var MacGlyphNamesIndices = make(map[string]int, NumberOfMacGlyphs)

func init() {
	for i, name := range MacGlyphNames {
		MacGlyphNamesIndices[name] = i
	}
}
