...
type recursiveXYCutTestCase struct {
	name     string
	expected []string
}

func recursiveXYCutTestData() []recursiveXYCutTestCase {
	return []recursiveXYCutTestCase{
		{
...
func resolveRecursiveXYCutDocPath(t *testing.T, name string) string {
	t.Helper()
...
func orderBlocksXYCut(blocks []*dla.TextBlock) []*dla.TextBlock {
...
func TestRecursiveXYCutGetBlocks(t *testing.T) {
	testCases := recursiveXYCutTestData()

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			docPath := resolveRecursiveXYCutDocPath(t, tc.name)
			...
			ordered := orderBlocksXYCut(blocks)

			for i, expectedText := range tc.expected {
				actual := strings.TrimSpace(ordered[i].Text)
				expectedTrimmed := strings.TrimSpace(expectedText)
				if actual != expectedTrimmed {
					t.Errorf("block %d:\nexpected: %q\n  actual: %q", i, expectedTrimmed, actual)
				}
			}
		})
	}
